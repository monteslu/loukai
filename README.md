...
- **Enable kiosk page (shared tablet)**: Serves an extra page at `/kiosk` for a device everyone shares, such as a tablet at the KJ booth. It works like the normal request page, except it never remembers anyone: there is no one-time name prompt, and every request asks who is singing, so a song can't be queued under the previous person's name. Off by default (`/kiosk` returns 404 until enabled).
