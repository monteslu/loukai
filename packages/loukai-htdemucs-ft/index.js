...
/** Absolute path to a bundled fine-tuned model (Node only), e.g. modelPath('vocals'). */
export function modelPath(stem) {
  return new URL(`./htdemucs_ft_${stem}_safe16.onnx`, import.meta.url).pathname;
}

/** Absolute path to the per-stem forceCpuNodeNames lists (Node only). */
export function cpuNodesPath() {
  return new URL('./ft_cpu_nodes.json', import.meta.url).pathname;
}
